...
namespace MakerShop.Utility
{
    ...
    public static class RecryptionHelper
    {
        ...
        public static int GetRecryptionWorkload()
        {
            ...
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_Payments WHERE Recrypt = 1");
            int totalCount = AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT FOR PAYMENTGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_PaymentGateways WHERE Recrypt = 1");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT FOR PAYMENTGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_ShipGateways WHERE Recrypt = 1");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT FOR PAYMENTGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_TaxGateways WHERE Recrypt = 1");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));
            return totalCount;
        }
        ...
        public static int EstimateRecryptionWorkload()
        {
            ...
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_Payments WHERE EncryptedAccountData IS NOT NULL");
            int totalCount = AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT CONFIGDATA IN PAYMENTGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_PaymentGateways WHERE ConfigData IS NOT NULL");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT CONFIGDATA IN SHIPGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_ShipGateways WHERE ConfigData IS NOT NULL");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));

            // COUNT CONFIGDATA IN TAXGATEWAYS TABLE
            selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_TaxGateways WHERE ConfigData IS NOT NULL");
            totalCount += AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));
            return totalCount;
        }
        ...
        public static void RecryptDatabase(HttpApplication context)
        {
            ...
            MakerShopEncryptionSection encryptionConfig = MakerShopEncryptionSection.GetSection(context);
        ...
        private static List<RecryptRecord> LoadForRecrypt(int maximumRows)
        {
            ...
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT TOP " + maximumRows + " P.PaymentId,P.EncryptedAccountData FROM ac_Payments P, ac_Orders O WHERE P.OrderId=O.OrderId AND O.StoreId=" + storeId + " AND P.Recrypt=1";
            ...
            if (remainingRows > 0)
            {
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT TOP " + remainingRows + " PaymentGatewayId,ConfigData FROM ac_PaymentGateways WHERE StoreId=" + storeId + " AND Recrypt=1";
            ...
            if (remainingRows > 0)
            {
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT TOP " + remainingRows + " ShipGatewayId,ConfigData FROM ac_ShipGateways WHERE StoreId=" + storeId + " AND Recrypt=1";
                selectCommand = database.GetSqlStringCommand(sql);
                using (IDataReader dr = database.ExecuteReader(selectCommand))
                {
                    while (dr.Read())
            ...
            if (remainingRows > 0)
            {
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT TOP " + remainingRows + " TaxGatewayId,ConfigData FROM ac_TaxGateways WHERE StoreId=" + storeId + " AND Recrypt=1";
                selectCommand = database.GetSqlStringCommand(sql);
                using (IDataReader dr = database.ExecuteReader(selectCommand))
                {
                    while (dr.Read())
