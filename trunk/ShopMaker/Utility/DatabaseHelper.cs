...
namespace MakerShop.Utility
{
    ...
    public static class DatabaseHelper
    {
        ...
        public static bool TableExists(string tableName)
        {
            ...
            Database database = Token.Instance.Database;
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) AS NumRows FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = @tableName";
        ...
        public static bool ColumnExists(string tableName, string columnName)
        {
            ...
            Database database = Token.Instance.Database;
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) AS NumRows FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @tableName AND COLUMN_NAME = @columnName";
            DbCommand command = database.GetSqlStringCommand(sql);
            database.AddInParameter(command, "tableName", DbType.String, tableName);
        ...
        public static bool ConstraintExists(string tableName, string constraintName)
        {
            ...
            Database database = Token.Instance.Database;
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) AS NumRows FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @tableName AND CONSTRAINT_NAME = @constraintName";
            DbCommand command = database.GetSqlStringCommand(sql);
            database.AddInParameter(command, "tableName", DbType.String, tableName);
        ...
        {
            Database database = Token.Instance.Database;
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT db_name()";
            DbCommand command = database.GetSqlStringCommand(sql);
        ...
        public static void UpdateConnectionString(string connectionString, bool encrypt) 
        {
            ...
            if (request != null)
            {
                try
                {
                    ...
                    section.ConnectionStrings["MakerShop"].ConnectionString = connectionString;
                    if (encrypt)
                    {
                ...
                catch
                {
                    ...
                    textBuilder.AppendFormat("\t<add name=\"MakerShop\" connectionString=\"{0}\" providerName=\"System.Data.SqlClient\" />\r\n", connectionString);
                    textBuilder.AppendLine("</connectionStrings>");
