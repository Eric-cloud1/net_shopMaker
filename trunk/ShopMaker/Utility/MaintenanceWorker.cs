...
namespace MakerShop.Utility
{
    ...
    public static class MaintenanceWorker
    {
        ...
        private static void MaintainCatalog()
        {
            ...
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT CatalogNodeId FROM ac_CatalogNodes CN LEFT JOIN ac_Products P ON CN.CatalogNodeId = P.ProductId WHERE P.ProductId IS NULL and CN.CatalogNodeTypeId = 1";
            ...
            sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT CatalogNodeId FROM ac_CatalogNodes CN LEFT JOIN ac_Webpages W ON CN.CatalogNodeId = W.WebpageId WHERE W.WebpageId IS NULL and CN.CatalogNodeTypeId = 2";
            ...
            sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT CatalogNodeId FROM ac_CatalogNodes CN LEFT JOIN ac_Links L ON CN.CatalogNodeId = L.LinkId WHERE L.LinkId IS NULL and CN.CatalogNodeTypeId = 3";
            idList = GetIdList(sql);
            foreach (int id in idList)
            {
            ...
            sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT CatalogNodeId FROM ac_CatalogNodes CN LEFT JOIN ac_Categories C ON CN.CatalogNodeId = C.CategoryId WHERE C.CategoryId IS NULL and CN.CatalogNodeTypeId = 0";
            idList = GetIdList(sql);
            foreach (int id in idList)
            {
            ...
            sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT CN.CategoryId, CN.CatalogNodeId FROM ac_CatalogNodes CN INNER JOIN ac_Categories C ON CN.CatalogNodeId=C.CategoryId WHERE CN.CatalogNodeTypeId=0 AND CN.CategoryID <> C.ParentID";
        ...
        private static void MaintainAnonymousCheckouts()
        {
            ...
            if (settings.AnonymousUserLifespan > 0)
            {
                ...
                string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT UserId FROM ac_Users WHERE StoreId = @storeId AND UserName LIKE 'zz_anonymous_%' AND (LastActivityDate IS NULL OR LastActivityDate < @expireDate)";
        ...
        private static void MaintainUsers()
        {
            ...
            if (settings.AnonymousUserLifespan > 0)
            {
                ...
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT UserId FROM ac_Users WHERE StoreId = @storeId AND IsAnonymous = 1 AND AffiliateId IS NULL AND ReferringAffiliateId IS NULL AND (LastActivityDate IS NULL OR LastActivityDate < @expireDate)";
                DbCommand dbCommand = database.GetSqlStringCommand(sql);
                database.AddInParameter(dbCommand, "@storeId", System.Data.DbType.Int32, store.StoreId);
                database.AddInParameter(dbCommand, "@expireDate", System.Data.DbType.DateTime, expireDate);
            ...
            if (settings.AnonymousAffiliateUserLifespan > 0)
            {
                ...
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT UserId FROM ac_Users WHERE StoreId = @storeId AND IsAnonymous = 1 AND (AffiliateId IS NOT NULL OR ReferringAffiliateId IS NOT NULL) AND (LastActivityDate IS NULL OR LastActivityDate < @expireDate)";
                DbCommand dbCommand = database.GetSqlStringCommand(sql);
                database.AddInParameter(dbCommand, "@storeId", System.Data.DbType.Int32, store.StoreId);
                database.AddInParameter(dbCommand, "@expireDate", System.Data.DbType.DateTime, expireDate);
                idList = GetIdList(dbCommand);
            ...
            if (policy.InactivePeriod > 0)
            {
                ...
                sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT UserId FROM ac_Users WHERE LastActivityDate < @expireDate AND IsApproved = 1 AND UserId IN (SELECT DISTINCT U.UserId FROM (ac_Users U INNER JOIN ac_UserGroups UG ON U.UserId = UG.UserId) INNER JOIN ac_GroupRoles GR ON UG.GroupId = GR.GroupId WHERE U.StoreId = @storeId)";
        ...
        private static void ClearAccountData()
        {
            ...
            selSql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT PaymentId ");
        ...
        {
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) As NumRecords FROM ac_PageViews WHERE StoreId = @storeId AND ActivityDate < @beforeDate";
            Database database = Token.Instance.Database;
        ...
        private static void SavePageViewLog(DateTime beforeDate)
        {
            try
            {
                ...
                {
                    //WRITE THE HEADER
                    fs.Write("#Software: MakerShop 7.0\r\n");
                    ...
                    string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT " + PageView.GetColumnNames(string.Empty) + " FROM ac_PageViews WHERE StoreId = @storeId AND ActivityDate < @beforeDate";
                    Database database = Token.Instance.Database;
                    DbCommand selectCommand = database.GetSqlStringCommand(sql);
                    database.AddInParameter(selectCommand, "@storeId", System.Data.DbType.Int32, Token.Instance.StoreId);
                    database.AddInParameter(selectCommand, "@beforeDate", System.Data.DbType.DateTime, beforeDate);
        ...
        {
            string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) As NumRecords FROM ac_AuditEvents WHERE StoreId = @storeId AND EventDate < @beforeDate";
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand(sql);
            database.AddInParameter(selectCommand, "@storeId", System.Data.DbType.Int32, Token.Instance.StoreId);
            database.AddInParameter(selectCommand, "@beforeDate", System.Data.DbType.DateTime, beforeDate);
        ...
        private static void SaveAuditLog(DateTime beforeDate)
        {
            try
            {
                ...
                using (StreamWriter fs = File.CreateText(fileName))
                {
                    ...
                    string sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT " + AuditEvent.GetColumnNames(string.Empty) + " FROM ac_AuditEvents WHERE StoreId = @storeId AND EventDate < @beforeDate";
                    Database database = Token.Instance.Database;
                    DbCommand selectCommand = database.GetSqlStringCommand(sql);
                    database.AddInParameter(selectCommand, "@storeId", System.Data.DbType.Int32, Token.Instance.StoreId);
                    database.AddInParameter(selectCommand, "@beforeDate", System.Data.DbType.DateTime, beforeDate);
