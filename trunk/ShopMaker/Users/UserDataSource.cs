...
namespace MakerShop.Users
{
    ...
    public partial class UserDataSource
    {
        ...
        public static string GetEmail(int userId)
        {
            ...
            DbCommand loadCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT Email FROM ac_Users WHERE UserId = @userId");
        ...
        {
            Database database = Token.Instance.Database;
            DbCommand loadCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT UserId FROM ac_Users WHERE LoweredUserName = @userName");
        ...
        public static UserCollection LoadForEmail(string email, bool includeAnonymous)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT " + User.GetColumnNames(""));
        ...
        public static int FindUsersByNameCount(string searchPattern, NameSearchField searchField)
        {
            ...
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) As RecordCount FROM " + tables);
            selectQuery.Append(whereClause);
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand(selectQuery.ToString());
            if (!string.IsNullOrEmpty(searchPattern))
        ...
        public static User CreateUser(string username, string email, string password, string passwordQuestion, string passwordAnswer, bool isApproved, int affiliateId, out System.Web.Security.MembershipCreateStatus status)
        {
            ...
            /*if (!anonUser)
            {
                ...
                DbCommand selectEmail = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT Email FROM ac_Users WHERE Email = @email");
            ...
            {
                //CREATE THE USER
                //encode the password and answer
                ...
                User user = new User();
                user.ReferringAffiliateId = affiliateId;
                user.AffiliateId = affiliateId;
                user.AffiliateReferralDate = LocaleHelper.LocalNow;
        ...
        private static User CreateUserInstance(string userName)
        {
            User user = new User();
            user.UserName = userName;
            user.IsAnonymous = (MakerShop.Utility.AlwaysConvert.ToGuid(userName) != Guid.Empty);
            user.CreateDate = LocaleHelper.LocalNow;
            HttpContext context = HttpContext.Current;
            bool foundCookies = false;
            if (context != null)
            {
                user.ReferringAffiliateId = AlwaysConvert.ToInt(context.Request.QueryString["afid"]);
                string cookieString = context.Request.ServerVariables["HTTP_COOKIE"];
                foundCookies = (!string.IsNullOrEmpty(cookieString));
            }
            //IF WE SAVE FOR ALL USERS HERE, IT WILL CREATE A USER RECORD FOR ALL ANONYMOUS HITS!
            if (foundCookies || user.ReferringAffiliateId > 0) user.Save();
            return user;
        }
        ...
        {
            Database database = Token.Instance.Database;
            DbCommand loadCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS UserCount FROM ac_Users WHERE UserId = @userId");
        ...
        public static User LoadForPayPalId(string payPalId)
        {
            //LOOKUP PayPalId
            Database database = Token.Instance.Database;
            DbCommand loadCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT UserId FROM ac_Users WHERE PayPalId = @payPalId");
        ...
        {
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(DISTINCT UR.UserId) AS TotalRecords FROM ac_UserRoles UR INNER JOIN ac_Subscriptions S ON UR.SubscriptionId = S.SubscriptionId WHERE S.SubscriptionPlanId = @subscriptionPlanId");
        ...
        public static int CountUsersByNameOrGroup(string searchPattern, NameSearchField searchField, int groupId, bool byName, bool includeAnonymous)
        {
            ...
            else
            {
                ...
                else
                {
                    Database database = Token.Instance.Database;
                    DbCommand selectCommand = null;
                    String sql ="SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;  SELECT COUNT(1) AS TotalRecords FROM ac_Users WHERE UserId NOT IN(SELECT UserId FROM ac_UserGroups)";
        ...
        public static bool Unsubscribe(string eMail, string referrer)
        {
            Database database = Token.Instance.Database;

            using (System.Data.Common.DbCommand cmd = database.GetStoredProcCommand("wsp_Unsubscribe"))
            {
                database.AddInParameter(cmd, "@eMail", System.Data.DbType.String, eMail);
                database.AddInParameter(cmd, "@Referrer", System.Data.DbType.String, referrer);
