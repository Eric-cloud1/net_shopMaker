...
namespace MakerShop.Users
{
    ...
    public partial class RoleDataSource
    {
        ...
        public static bool IsUserInRole(Int32 userId, Int32 roleId)
        {
            ...
            if (HttpContext.Current != null)
            {
                ...
                if (userRoles == null)
                {
                    ...
                    selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT GR.RoleId");
            ...
            else
            {
                ...
                StringBuilder selectQuery = new StringBuilder();
                selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS RoleCount");
                selectQuery.Append(" FROM ac_UserGroups UG INNER JOIN ac_GroupRoles GR ON UG.GroupId = GR.GroupId");
                selectQuery.Append(" WHERE UG.UserId = @userId");
        ...
        {
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_Roles");
        ...
        public static RoleCollection LoadForUser(Int32 userId)
        {
            //CREATE THE DYNAMIC SQL TO LOAD OBJECT
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT " + Role.GetColumnNames("R"));
