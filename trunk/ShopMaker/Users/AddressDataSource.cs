...
namespace MakerShop.Users
{
    ...
    public partial class AddressDataSource
    {
        ...
        {
            Database database = Token.Instance.Database;
            using (DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT PrimaryAddressId FROM ac_Users WHERE UserId = @userId"))
            {
        ...
        {
            Database database = Token.Instance.Database;
            using (DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT MIN(AddressId) As FirstId FROM ac_Addresses WHERE UserId = @userId"))
            {
                database.AddInParameter(selectCommand, "@userId", DbType.Int32, userId);
                return AlwaysConvert.ToInt(Token.Instance.Database.ExecuteScalar(selectCommand));
            }
        ...
        {
            Database database = Token.Instance.Database;
            using (DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS NumAddr FROM ac_Addresses WHERE AddressId = @addressId AND UserId = @userId"))
            {
