...
namespace MakerShop.Taxes
{
    ...
    public partial class TaxRuleDataSource
    {
        ...
        public static int GetTaxRuleIdByName(string name)
        {
            ...
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT TaxRuleId FROM ac_TaxRules WHERE StoreId=@storeId AND UPPER(Name)=@NAME");
        ...
        public static TaxRuleCollection LoadForBasket(int basketId, TaxAddress address, User user)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT TaxCodeId FROM ac_BasketItems");
        ...
        public static TaxRuleCollection LoadForTaxCodes(int[] taxCodeIds, TaxAddress taxAddress, User user)
        {
            ...
            ShipZoneCollection shipmentZoneCollection = ShipZoneDataSource.LoadForAddress(taxAddress);

            //BUILD A LIST OF ZONEIDS FOR QUERY CRITERIA
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT " + TaxRule.GetColumnNames("TR"));
