...
namespace MakerShop.Personalization
{
    ...
    public partial class ProfileDataSource
    {
        ...
        public static int DeleteInactiveProfiles(ProfileAuthenticationOption authenticationOption, DateTime userInactiveSinceDate)
        {
            ...
            int rtn = database.ExecuteNonQuery(deleteCommand);
        ...
        {
            string sqlQuery = "SELECT COUNT(1) FROM ac_Users U, ac_Profiles P WHERE U.StoreId = @storeId AND U.LastActivityDate <= @lastActivityDate AND U.UserId = P.UserId " + GetClauseForAuthenticationOptions(authenticationOption);
            Database database = Token.Instance.Database;
