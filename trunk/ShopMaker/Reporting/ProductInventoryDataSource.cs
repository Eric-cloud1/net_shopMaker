...
namespace MakerShop.Reporting
{
    ...
    public static class ProductInventoryDataSource
    {
        ...
        public static List<ProductInventoryDetail> GetLowProductInventory(int maximumRows, int startRowIndex, string sortExpression)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT ProductId, Name, NULL AS ProductVariantId, NULL As VariantName, InStock, InStockWarningLevel");
        ...
        {
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS SubTotal");
            selectQuery.Append(" FROM ac_Products");
            selectQuery.Append(" WHERE InStock <= InStockWarningLevel");
            selectQuery.Append(" AND InventoryModeId = 1");
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand(selectQuery.ToString());
            int count1 = MakerShop.Utility.AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));
            ...
            int count2 = MakerShop.Utility.AlwaysConvert.ToInt(database.ExecuteScalar(selectCommand));
        ...
        {
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT SUM(SubTotal) AS Total FROM (SELECT COUNT(1) AS SubTotal");
            selectQuery.Append(" FROM ac_Products");
            selectQuery.Append(" WHERE InStock <= @maxInStock");
            selectQuery.Append(" AND InventoryModeId = 1");
            selectQuery.Append(" UNION");
            selectQuery.Append(" SELECT COUNT(1) AS SubTotal");
            selectQuery.Append(" FROM ac_ProductVariants V, ac_Products P");
            selectQuery.Append(" WHERE V.ProductId = P.ProductId");
            selectQuery.Append(" AND P.InventoryModeId = 2");
