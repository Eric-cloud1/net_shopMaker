...
namespace MakerShop.Reporting
{
    ...
    public partial class ReportDataSource
    {
        ...
        public static List<UserSummary> GetSalesByUser(DateTime startDate, DateTime endDate, int maximumRows, int startRowIndex, string sortExpression)
        {
            ...
            selectQuery.Append(" UserId, COUNT(1) AS OrderCount, SUM(TotalCharges) AS OrderTotal");
        ...
        public static int GetSalesByUserCount(DateTime startDate, DateTime endDate)
        {
            ...
            Database database = Token.Instance.Database;
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS UserCount FROM (SELECT DISTINCT UserId FROM ac_Orders");
            selectQuery.Append(" WHERE StoreId = @storeId");
            selectQuery.Append(" AND " + GetStatusFilter(reportStatuses));
            if (startDate > DateTime.MinValue)
            {
        ...
        public static List<CouponSummary> GetSalesByCoupon(DateTime startDate, DateTime endDate, string couponCode, int maximumRows, int startRowIndex, string sortExpression)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT");
            if (maximumRows > 0) selectQuery.Append(" TOP " + (startRowIndex + maximumRows).ToString());
            selectQuery.Append(" ac_OrderCoupons.CouponCode, COUNT(1) AS OrderCount, SUM(TotalCharges) AS OrderTotal");
        ...
        public static int GetSalesByCouponCount(DateTime startDate, DateTime endDate, string couponCode)
        {
            ...
            Database database = Token.Instance.Database;
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS CouponCount FROM (SELECT DISTINCT CouponCode FROM ac_Orders INNER JOIN ac_OrderCoupons ON ac_Orders.OrderId = ac_OrderCoupons.OrderId");
            selectQuery.Append(" WHERE StoreId = @storeId");
            selectQuery.Append(" AND " + GetStatusFilter(reportStatuses));
            if (startDate > DateTime.MinValue)
            {
        ...
        public static int GetSalesByProductCount(DateTime startDate, DateTime endDate)
        {
            ...
            Database database = Token.Instance.Database;
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS ProductCount FROM (SELECT DISTINCT P.ProductId FROM (ac_Orders O INNER JOIN ac_OrderItems OI ON O.OrderId = OI.OrderId) INNER JOIN ac_Products P ON OI.ProductId = P.ProductId");
            selectQuery.Append(" WHERE O.StoreId = @storeId");
            selectQuery.Append(" AND " + GetStatusFilter(reportStatuses, "O"));
            if (startDate > DateTime.MinValue)
            {
        ...
        public static SalesSummary GetSalesSummary(DateTime startDate, DateTime endDate, bool updateUserCount)
        {
            ...
            if (summary.OrderCount > 0)
            {
                ...
                sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT OI.OrderItemTypeId, SUM(OI.Price * OI.Quantity), SUM(OI.CostOfGoods * OI.Quantity), SUM(OI.Quantity)");
                ...
                if (updateUserCount)
                {
                    ...
                    sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT UserId FROM ac_Orders");
        ...
        private static int GetOrderCount(DateTime startDate, DateTime endDate, List<OrderStatus> reportStatuses)
        {
            ...
            if (endDate != DateTime.MinValue) if (endDate.Kind != DateTimeKind.Utc) endDate = LocaleHelper.FromLocalTime(endDate);
            StringBuilder sql = new StringBuilder();
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS OrderCount");
        ...
        public static List<OrderSummary> GetDailySales(int year, int month, int day)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT O.OrderId, O.OrderNumber, OI.OrderItemTypeId, SUM(OI.Price * OI.Quantity), SUM(OI.CostOfGoods * OI.Quantity)");
            sql.Append(" FROM ac_Orders O INNER JOIN ac_OrderItems OI ON O.OrderId = OI.OrderId");
            sql.Append(" WHERE O.StoreId = @storeId");
            sql.Append(" AND " + GetStatusFilter(reportStatuses, "O"));
        ...
        private static AbandonedBasketsSummary GetAbandonedBasketsSummary(DateTime startDate, DateTime endDate)
        {
            ...
            bSql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT b.BasketId FROM ac_Baskets b INNER JOIN ac_Users u on b.UserId=u.UserId ");
        ...
        public static List<BasketSummary> GetDailyAbandonedBaskets(int year, int month, int day)
        {
            ...
            StringBuilder bSql = new StringBuilder();
            bSql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT b.BasketId, u.UserName, u.LastActivityDate FROM ac_Baskets b INNER JOIN ac_Users u on b.UserId=u.UserId ");
            bSql.Append(" WHERE u.StoreId = @storeId");
            bSql.Append(" AND u.LastActivityDate >= @startDate");
            bSql.Append(" AND u.LastActivityDate < @endDate");
            bSql.Append(" AND u.LastActivityDate < @thirtMinAgo");
        ...
        [System.ComponentModel.DataObjectMethod(System.ComponentModel.DataObjectMethodType.Select)]
        public static DataTable GetSalesByAffiliateSummary(DateTime startDate, DateTime endDate, int? affiliateId)
        {
            Database database = Token.Instance.Database;
            DataTable dt = new DataTable();
            using (System.Data.Common.DbCommand cmd = database.GetStoredProcCommand("rpt_GetAffiliateSummary"))
            {
                ...
                if (affiliateId.HasValue)
                    database.AddInParameter(cmd, "@AffiliateId", DbType.Int32, affiliateId);
        ...
        public static List<AffiliateSalesSummary> GetSalesByAffiliate(DateTime startDate, DateTime endDate, int affiliateId)
        {
            ...
            //CREATE THE DYNAMIC SQL TO LOAD OBJECT
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT AffiliateId, COUNT(1) AS OrderCount, SUM(ProductSubtotal) AS OrderSubtotal, SUM(TotalCharges) AS OrderTotal, COUNT(CASE WHEN OrderStatusId = 3 then 1 end) as Successful, COUNT(CASE when OrderStatusId = 2 then 1 end) as Pending");
            selectQuery.Append(" FROM ac_Orders");
            selectQuery.Append(" WHERE StoreId = @storeId");
            ...
            using (IDataReader dr = database.ExecuteReader(selectCommand))
            {
                while (dr.Read())
                {
                    summary = new AffiliateSalesSummary(dr.GetInt32(0), startDate, endDate, dr.GetInt32(1), dr.GetDecimal(2), dr.GetDecimal(3), dr.GetInt32(4), dr.GetInt32(5));
            ...
            if (affiliateId > 0)
            {
                ...
                else summary = new AffiliateSalesSummary(affiliate.AffiliateId, startDate, endDate, 0, 0, 0, 0,0);
        ...
        public static int GetSalesByAffiliateCount(DateTime startDate, DateTime endDate)
        {
            ...
            Database database = Token.Instance.Database;
            StringBuilder selectQuery = new StringBuilder();
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS AffiliateCount FROM (SELECT DISTINCT AffiliateId FROM ac_Orders");
            selectQuery.Append(" WHERE StoreId = @storeId");
        ...
        public static SortableCollection<KeyValuePair<DateTime, decimal>> GetSalesForPastMonths(int monthCount, bool includeZeros)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DATEPART(mm, DATEADD(hh, " + hourOffset + ", OrderDate)) AS OrderMonth, SUM(TotalCharges) AS MonthlySales");
            sql.Append(" FROM ac_Orders");
            sql.Append(" WHERE StoreId = @storeId");
            sql.Append(" AND " + GetStatusFilter(reportStatuses));
        ...
        public static SortableCollection<KeyValuePair<DateTime, decimal>> GetSalesForPastDays(int dayCount, bool includeZeros)
        {
            ...
            //BUILD QUERY TO GET SALES ARRANGED BY MONTH
            StringBuilder sql = new StringBuilder();
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DATEPART(dd, DATEADD(hh, " + hourOffset + ", OrderDate)) AS OrderDay, SUM(TotalCharges) AS DailySales");
            sql.Append(" FROM ac_Orders");
            sql.Append(" WHERE StoreId = @storeId");
            sql.Append(" AND " + GetStatusFilter(reportStatuses));
            sql.Append(" AND OrderDate >= @startDate");
        ...
        /// <summary>
        /// Gets sales totals Quantity for the past days
        /// </summary>
        /// <param name="dayCount">Can be between 1 to 15</param>
        /// <param name="includeZeros">Whether to include zero amount sales or not</param>
        /// <returns>A collection of Key-Value pairs with each key representing the date(day) and each value representing the total sale</returns>
        public static SortableCollection<KeyValuePair<DateTime, decimal>> GetSalesQtyForPastDays(int dayCount, bool includeZeros)
        {
            if ((dayCount < 0) || (dayCount > 15)) throw new ArgumentOutOfRangeException("dayCount", "dayCount must be between 1 and 15");
            //DETERMINE THE START MONTH
            dayCount -= 1;
            ...
            //BUILD QUERY TO GET SALES ARRANGED BY MONTH
            StringBuilder sql = new StringBuilder();
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DATEPART(dd, DATEADD(hh, " + hourOffset + ", OrderDate)) AS OrderDay, SUM(Quantity) AS DailySales");
            sql.Append(" FROM ac_Orders o INNER JOIN ac_OrderItems i on i.OrderId = o.OrderId");
            sql.Append(" WHERE o.StoreId = @storeId AND i.ProductId is not null");
            sql.Append(" AND " + GetStatusFilter(reportStatuses));
            sql.Append(" AND o.OrderDate >= @startDate");
            sql.Append(" GROUP BY DATEPART(dd, DATEADD(hh, " + hourOffset + ", o.OrderDate))");
            //LOAD TABLE OF ORDERITEM TOTALS
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand(sql.ToString());
            database.AddInParameter(selectCommand, "@storeId", System.Data.DbType.Int32, Token.Instance.StoreId);
            ...
            using (IDataReader dr = database.ExecuteReader(selectCommand))
            {
                while (dr.Read())
                {
                    ...
                    DateTime key = new DateTime(year, month, day, 0, 0, 0);
                    sparseArray.Add(new KeyValuePair<DateTime, decimal>(key, dr.GetInt32(1)));
                }
                dr.Close();
            }
            //SORT THE RESULTS BY MONTH ASCENDING
        ...
        public static List<ProductBreakdownSummary> GetProductBreakdownSummary(DateTime fromDate, DateTime toDate, int vendorId, string sortExpression)
        {
            ...
            if (fromDate != DateTime.MinValue || toDate != DateTime.MinValue)
            {
                ...
                if (toDate != DateTime.MinValue)
                {
                    ...
                    toDate1 = new DateTime(toDate.Year, toDate.Month, toDate.Day, 23, 59, 59, 999);
                    toDate1 = LocaleHelper.FromLocalTime(toDate1);
                }
            }
            sqlBuilder.Append(" ) ");
            ...
            if (toDate != DateTime.MinValue)
                database.AddInParameter(selectCommand, "@TD", System.Data.DbType.DateTime, toDate1);
            //EXECUTE THE COMMAND
            using (IDataReader dr = database.ExecuteReader(selectCommand))
            {
                ...
                while (dr.Read())
                {
                    ...
                    product = MakerShop.Products.ProductDataSource.Load(AlwaysConvert.ToInt(productId));
        ...
        public static List<ReferrerSalesSummary> GetSalesByReferrer(DateTime startDate, DateTime endDate, int maximumRows, int startRowIndex, string sortExpression)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT");
            if (maximumRows > 0) selectQuery.Append(" TOP " + (startRowIndex + maximumRows).ToString());
            selectQuery.Append(" Referrer, COUNT(1) AS OrderCount, SUM(ProductSubtotal) AS ProductSubtotal, SUM(TotalCharges) AS OrderTotal");
            selectQuery.Append(" FROM ac_Orders");
            selectQuery.Append(" WHERE StoreId = @storeId");
