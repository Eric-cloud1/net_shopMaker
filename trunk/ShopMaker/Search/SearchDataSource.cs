...
namespace MakerShop.Search
{
    ...
    public partial class SearchDataSource
    {
        ...
        public static List<SearchAreaResults> Search(String keywords, List<SearchArea> searchAreas, int maximumRows)
        {
            ...
            {                
                fixedKeywords = StringHelper.FixSearchPattern(keywords).ToLowerInvariant();
            }
            ...
            searchAreaDetails.Add(new SearchAreaDetail(SearchArea.Products, "ac_Products", "ProductId", "Name", new string[] { "Name", "Sku", "Description" }));
            ...
            foreach (SearchAreaDetail searchAreaDetail in searchAreaDetails)
            {
                if (searchAreas.Contains(searchAreaDetail.SearchArea) || searchAreas.Contains(SearchArea.All))
                {
                    ...
                    if (maximumRows > 0) sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT TOP ").Append(maximumRows).Append(" ");
                    else sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT ");
                    ...
                    if (searchAreaResults.SearchResults.Count > 0)
                    {
                        if (maximumRows > 0 && searchAreaResults.SearchResults.Count >= maximumRows)
                        {
                            // COUNT ALL RESULTS
                            sqlBuilder = new StringBuilder();
                            sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT Count(").Append(searchAreaDetail.KeyField).Append(")");
            ...
            if (searchAreas.Contains(SearchArea.Users) || searchAreas.Contains(SearchArea.All))
            {
                ...
                sqlBuilder = new StringBuilder();
                if (maximumRows > 0) sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT TOP ").Append(maximumRows);
                else sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT ");
                ...
                if (searchAreaResults.SearchResults.Count > 0)
                {
                    ...
                    {
                        sqlBuilder = new StringBuilder();
                        sqlBuilder.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT Count(U.UserId)");
