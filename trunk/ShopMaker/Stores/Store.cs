...
namespace MakerShop.Stores
{
    ...
    public partial class Store
    {
        ...
        public static StoreSettingCollection GetCachedSettings()
        {
            ...
            if (context != null)
            {
                StoreSettingCollection settings = context.Cache["MakerShopStoreSettings"] as StoreSettingCollection;
                if (settings == null)
                {
                    settings = Token.Instance.Store.Settings;
                    context.Cache.Insert("MakerShopStoreSettings", settings, null, DateTime.Now.AddMinutes(20), Cache.NoSlidingExpiration, CacheItemPriority.High, null);
                }
                return settings;
            }
        ...
        {
            HttpContext context = HttpContext.Current;
            if (context != null) context.Cache.Remove("MakerShopStoreSettings");
        }
        ...
        public SaveResult Save()
        {
            ...
            if (this.IsDirty)
            {
                ...
                if (recordExists)
                {
                    ...
                    selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) As RecordCount FROM ac_Stores");
