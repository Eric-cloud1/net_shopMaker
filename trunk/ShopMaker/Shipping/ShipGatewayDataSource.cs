...
namespace MakerShop.Shipping
{
    ...
    public partial class ShipGatewayDataSource
    {
        ...
        public static int GetShipGatewayIdByClassId(string classId)
        {
            ...
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT ShipGatewayId FROM ac_ShipGateways WHERE StoreId=@storeId AND ClassId=@classId");
        ...
        public static ShipGatewayCollection LoadForClassId(string classId)
        {
            ...
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT " + ShipGateway.GetColumnNames(string.Empty) + "  FROM ac_ShipGateways WHERE StoreId=@storeId AND ClassId=@classId");
            database.AddInParameter(selectCommand, "@storeId", DbType.Int32, Token.Instance.StoreId);
            database.AddInParameter(selectCommand, "@classId", DbType.String, classId);
            //EXECUTE THE COMMAND
