...
namespace MakerShop.Orders
{
    ...
    public partial class OrderItemDataSource
    {
        ...
        public static List<OrderItem> CreateForProduct(int productId, short quantity, MakerShop.Payments.PaymentTypes paymentType)
        {
            return OrderItemDataSource.CreateForProduct(productId, quantity, string.Empty, null, paymentType);
        }


        /// <summary>
        /// Gets related order items that would result from the addition of the given product to an order.
        /// </summary>
        /// <param name="productId">The id of the product to add.</param>
        /// <param name="quantity">The quantity of the product to add.</param>
        /// <returns>The order items that would result from the addition of the given product to an order</returns>
        ...
        public static DataTable LoadRelatedOrders(int orderId)
        {
                 
            DataSet relatedOrders = new DataSet();
            Database database = Token.Instance.Database;

            using (System.Data.Common.DbCommand dataSetCommand = database.GetStoredProcCommand("rpt_RelatedOrders"))
            {
                database.AddInParameter(dataSetCommand, "@OrderId", System.Data.DbType.Int32, orderId);

     
                relatedOrders = database.ExecuteDataSet(dataSetCommand);
            }

            return relatedOrders.Tables[0];
        }
        ...
        /// <returns>The order items that would result from the addition of the given product to an order</returns>
        public static List<OrderItem> CreateForProduct(int productId, short quantity, string optionList, string kitList, MakerShop.Payments.PaymentTypes paymentType)
        {
            ...
            if (product != null)
            {
                ...
                ProductCalculator pcalc = ProductCalculator.LoadForProduct(productId, quantity, optionList, kitList,paymentType);
        ...
        public static OrderItemCollection LoadSubscriptionItems(int orderId)
        {
            ...
            selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT " + OrderItem.GetColumnNames("OI"));
