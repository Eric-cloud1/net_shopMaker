...
namespace MakerShop.Orders
{
    ...
    public partial class OrderItemDigitalGood
    {
        ...
        /// <summary>
        /// Upgrade a serial key for this digital good
        /// </summary>
        public void UpgradeSerialKey(string newLevel)
        {
            if (IsSerialKeyAcquired())
            {
                return;
            ...
            ISerialKeyProvider provider = DigitalGood.GetSerialKeyProviderInstance();
            AcquireSerialKeyResponse response = provider.UpgradeSerialKey(this, newLevel);
            ...
            {
                //serial key could not be acquired.
                Logger.Error(string.Format("Serial Key could not be upgraded for {0} using Serial Key Provider {1}. Provider Error Message '{2}'", DigitalGood.Name, provider.Name, response.ErrorMessage));
            }
        }
        ...
        public virtual bool Load(string eMail, string postalCode, int digitalGoodId)
        {
            Database database = Token.Instance.Database;
            bool result = false;
            using (System.Data.Common.DbCommand selectCommand = database.GetStoredProcCommand("wsp_OrderItemDigitalGood"))
            {
                database.AddInParameter(selectCommand, "@Email", System.Data.DbType.String, eMail);
                database.AddInParameter(selectCommand, "@PostalCode", System.Data.DbType.String, postalCode);
