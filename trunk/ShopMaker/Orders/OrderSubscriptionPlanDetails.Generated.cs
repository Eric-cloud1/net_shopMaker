...
namespace MakerShop.Orders
{
    ...
    /// </summary>
    public partial class OrderSubscriptionPlanDetails : IPersistable
    {

        #region Constructors

        public OrderSubscriptionPlanDetails() { }

        public OrderSubscriptionPlanDetails(Int32 pOrderId, Int16 pPaymentTypeId)
        {
        ...
        public static void LoadDataReader(OrderSubscriptionPlanDetails pOrderSubscriptionPlanDetails, IDataReader dr)
        {
            //SET FIELDS FROM ROW DATA

            pOrderSubscriptionPlanDetails.OrderId = dr.GetInt32(0);

            pOrderSubscriptionPlanDetails.PaymentTypeId = dr.GetInt16(1);

            pOrderSubscriptionPlanDetails.PaymentAmount = dr.GetDecimal(2);

            pOrderSubscriptionPlanDetails.ShipMethodId = dr.GetInt32(3);

            pOrderSubscriptionPlanDetails.SplitShip = dr.GetBoolean(4);

            pOrderSubscriptionPlanDetails.PaymentDays = dr.GetInt16(5);

            pOrderSubscriptionPlanDetails.NumberOfPayments = dr.GetInt16(6);

            pOrderSubscriptionPlanDetails.DaysToCapture = dr.GetInt16(7);

            pOrderSubscriptionPlanDetails.CreateDate = LocaleHelper.ToLocalTime(dr.GetDateTime(8));

            pOrderSubscriptionPlanDetails.CreateUser = dr.GetString(9);

            pOrderSubscriptionPlanDetails.ChangeDate = LocaleHelper.ToLocalTime(dr.GetDateTime(10));

            pOrderSubscriptionPlanDetails.ChangeUser = dr.GetString(11);


            pOrderSubscriptionPlanDetails.IsDirty = false;
        }
        ...
        public virtual bool Delete()
        {
            ...
            deleteQuery.Append("DELETE FROM xm_OrderSubscriptionPlanDetails");
        ...
        public virtual SaveResult Save()
        {
            if (this.IsDirty)
            {
                ...
                if (recordExists)
                {
                    ...
                    StringBuilder selectQuery = new StringBuilder();
                    selectQuery.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) As RecordCount FROM xm_OrderSubscriptionPlanDetails");
                    selectQuery.Append(" WHERE  OrderId = @OrderId AND  PaymentTypeId = @PaymentTypeId ");
                ...
                if (recordExists)
                {
                    ...
                    updateQuery.Append("UPDATE xm_OrderSubscriptionPlanDetails SET ");

                    updateQuery.Append("PaymentAmount = @PaymentAmount,ShipMethodId = @ShipMethodId,SplitShip = @SplitShip,PaymentDays = @PaymentDays,NumberOfPayments = @NumberOfPayments,CreateDate = @CreateDate,CreateUser = @CreateUser,ChangeDate = @ChangeDate,ChangeUser = @ChangeUser, DaysToCapture=@DaysToCapture");
                ...
                else
                {
                    ...
                    insertQuery.Append("INSERT INTO xm_OrderSubscriptionPlanDetails (OrderId,PaymentTypeId,PaymentAmount,ShipMethodId,SplitShip, DaysToCapture,PaymentDays,NumberOfPayments,CreateDate,CreateUser,ChangeDate,ChangeUser )");
                    insertQuery.Append(" VALUES (@OrderId,@PaymentTypeId,@PaymentAmount,@ShipMethodId,@SplitShip,@DaysToCapture,@PaymentDays,@NumberOfPayments,@CreateDate,@CreateUser,@ChangeDate,@ChangeUser )");
