...
namespace MakerShop.Orders
{
    ...
    internal static class CheckoutHelper
    {
        ...
        internal static Order GenerateOrderObject(Basket basket, string customerIP)
        {
            ...
            if (affiliate != null)
            {
                //CHECK IF AFFILIATE REFERRAL IS PERSISTENT OR STILL WITHIN REFERRAL PERIOD
                if ((affiliate.ReferralDays == 0) || (user.AffiliateReferralDate > LocaleHelper.LocalNow.AddDays(-1 * affiliate.ReferralDays)))
                {
                    //THE REFERRAL IS STILL VALID
                    order.AffiliateId = affiliate.AffiliateId;
                }
                else
                {
                    ResetUserAffiliate(user);
                }
            }
        ...
        internal static void GenerateOrderItemDigitalGoods(Order order)
        {
            ...
            try
            {
                string query = "SELECT OI.OrderItemId, OI.Quantity, PDG.DigitalGoodId FROM ac_OrderItems OI JOIN ac_Products P ON OI.ProductId = P.ProductId JOIN ac_ProductDigitalGoods PDG ON P.ProductId = PDG.ProductId ";
                query += " WHERE OrderId = @orderId AND ((PDG.OptionList IS NULL) OR (OI.OptionList = PDG.OptionList))";
        ...
        internal static void GenerateOrderPayments(Order order, CheckoutRequest checkoutRequest, List<BasketPaymentItem> giftCertPayments, int giftCertPaymentMethodId, Dictionary<int, Subscription[]> orderItemSubscriptions)
        {
            ...
            if (checkoutRequest != null && checkoutRequest.Payment != null)
            {
                ...
                SubscriptionPlanDetails spd = null;
                if (productId.HasValue)
                    spd = SubscriptionPlanDetailsDataSource.Load(productId.Value, originalPayment.PaymentTypeId);
                ...
                if (spd != null)
                {
                    ShipMethod sm = ShipMethodDataSource.Load(spd.ShipMethodId);
                    LSDecimal shiprate = 0;
                    if (sm.ShipRateMatrices.Count > 0)
                        shiprate = sm.ShipRateMatrices[0].Rate;

                    if (spd.SplitShip)
                    {
                        if (shiprate > 0)
                        {
                            // make shipment it's own payment
                            Payment arbPayment = new Payment();
                            //arbPayment.SubscriptionId = s.SubscriptionId;
                            arbPayment.Amount = sm.ShipRateMatrices[0].Rate;
                            ...
                            arbPayment.PaymentTypeId = originalPayment.PaymentTypeId;
                            arbPayment.PaymentGatewayId = originalPayment.PaymentGatewayId;
                            arbPayment.PaymentInstrumentId = originalPayment.PaymentInstrumentId;
                        ...
                        // remove shipment amount from original payment
                        originalPayment.Amount = spd.PaymentAmount;
                    }
                    else
                    {

                        originalPayment.Amount = shiprate + spd.PaymentAmount;
                    }
                ...
                else if (orderItemSubscriptions.Count > 0)
                {
                    ...
                    if (remainingPaymentAmount > 0)
                    {
                        ...
                        remainingPayment.PaymentTypeId = originalPayment.PaymentTypeId;
                        remainingPayment.PaymentGatewayId = originalPayment.PaymentGatewayId;
                        remainingPayment.PaymentInstrumentId = originalPayment.PaymentInstrumentId;
        ...
        /// of tax reporting.  The native MakerShop provider does not require this step.</remarks>
