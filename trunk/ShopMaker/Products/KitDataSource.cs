...
namespace MakerShop.Products
{
    ...
    public partial class KitDataSource
    {
        ...
        public static int CountForStore()
        {
            ...
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_Products WHERE StoreId = @storeId AND ProductId IN (SELECT DISTINCT ProductId FROM ac_ProductKitComponents)");
        ...
        public static int FindByNameCount(string nameToMatch)
        {
            ...
            Database database = Token.Instance.Database;
            DbCommand selectCommand = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT COUNT(1) AS TotalRecords FROM ac_Products WHERE StoreId = @storeId AND ProductId IN (SELECT DISTINCT ProductId FROM ac_ProductKitComponents) AND (Name LIKE @nameToMatch)");
            database.AddInParameter(selectCommand, "@storeId", System.Data.DbType.Int32, Token.Instance.StoreId);
