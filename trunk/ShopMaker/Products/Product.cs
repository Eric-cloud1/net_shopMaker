...
namespace MakerShop.Products
{
    ...
    public partial class Product : CatalogableBase
    {
        ...
        private ProductCommission _ProductCommission;

        /// <summary>
        /// related productsCommission
        /// </summary>
        public ProductCommission ProductCommission
        {
            get
            {
                if (_ProductCommission == null)
                {
                    _ProductCommission = ProductCommissionDataSource.Load(this.ProductId);

                    if (_ProductCommission == null)
                        _ProductCommission = new ProductCommission();

                }
                return _ProductCommission;
            }
        }
        ...
        public void SaveCopy(int categoryId, string copyName)
        {
            ...
            SubscriptionPlanDetailsCollection subplandetails = this.SubscriptionPlanDetails;
            SubscriptionPlanDownsellsCollection downSells = this.SubscriptionPlanDownsellsCollection;
            PaymentGatewayTemplate_Product gateway = this.PaymentGatewayTemplate_Product;
            ...
            if (subplan != null)
            {
                subplan.IsDirty = true;
                subplan.ProductId = this.ProductId;
                subplan.Save();
            }
            for (int i = 0; i < downSells.Count; i++)
            {
                downSells[i].ProductId = this.ProductId;
                downSells[i].Save();
            }
            if (gateway != null)
            {
                gateway.IsDirty = true;
                gateway.ProductId = this.ProductId;
                gateway.Save();
            }

            foreach (SubscriptionPlanDetails subs in this.SubscriptionPlanDetails)
            {
                subs.ProductId = this.ProductId;
