...
namespace MakerShop.Messaging
{
    ...
    public class MailMergeTemplate
    {
        ...
        private MailMessage[] GenerateMessages(MailMergeRecipientCollection recipients)
        {
        ...
        public void Send(MailMergeRecipientCollection recipients, bool async)
        {
            ...
            try
            {
                messages = this.GenerateMessages(recipients);
            }
            catch (Exception ex)
            {
                Logger.Error("Error generating batch messages having subject '" + this.Subject + "'.", ex);
            }
