...
namespace MakerShop.Marketing
{
    ...
    public class DiscountCalculator
    {
        ...
        private static BasketItemCollection GetCategoryItems(int categoryId, Basket basket, List<int> productsToExclude)
        {
            ...
            categorySql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT BI.BasketItemId");
        ...
        private static int[] GetGiftCertificateProductIds(int basketId)
        {
            ...
            sql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT BI.ProductId");
        ...
        private static DiscountedBasketProduct[] GetDiscountedBasketProducts(int basketId, List<int> productsToExclude)
        {
            ...
            //EXECUTE THE QUERY
            Database database = Token.Instance.Database;
            DbCommand command = database.GetSqlStringCommand("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT ProductId,AVG(Price),SUM(Quantity) FROM ac_BasketItems WHERE BasketId = @basketId AND ProductId IN (SELECT DISTINCT ProductId FROM ac_ProductVolumeDiscounts" + excludedProducts + ") GROUP BY ProductId");
            database.AddInParameter(command, "@basketId", System.Data.DbType.Int32, basketId);
        ...
        private static Dictionary<int, List<PotentialDiscount>> GetPotentialDiscounts(Basket basket, List<int> productsToExclude, GroupingMode groupingMode)
        {
            ...
            StringBuilder categorySql = new StringBuilder();
            categorySql.Append("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED; SELECT DISTINCT CP.ParentId, CP.ParentLevel, CVD.VolumeDiscountId");
