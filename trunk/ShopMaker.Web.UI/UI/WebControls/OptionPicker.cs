        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (_Option != null)
            {
                ...
                foreach (OptionChoice choice in _Option.Choices)
                {
                    ...
                    {
                        imageScript.Append("    " + this.ClientID + "_Images[" + choice.OptionChoiceId.ToString() + "] = '" + this.Page.ResolveClientUrl(choice.ImageUrl) + "';\n");
                    }
                }
            }
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            if (this.RenderOptions)
            {
                if (_Option != null)
                {
                    ...
                    OptionChoiceCollection availableChoices = OptionChoiceDataSource.GetAvailableChoices(productId, _Option.OptionId, _SelectedChoices);
                    if (availableChoices.Count > 0)
                    {
                        ...
                        for (int i = 0; i < availableChoices.Count; i++)
                        {
                            ...
                            html.Append(" style=\"background-image:url(" + this.Page.ResolveClientUrl(choice.ThumbnailUrl) + ");z-index:" + choice.OptionChoiceId.ToString() + "\">&nbsp;</td>\n");
                        }
                        html.Append("            </tr>\n");
                        html.Append("     </table>\n");
                        html.Append("</div>\n");
