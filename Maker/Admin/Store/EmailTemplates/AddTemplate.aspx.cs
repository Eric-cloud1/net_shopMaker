...
public partial class Admin_Store_EmailTemplates_AddTemplate : MakerShop.Web.UI.MakerShopAdminPage
{
        ...
        if (!Page.IsPostBack)
        {
            ...
            else
            {
                ...
                foreach (GridViewRow row in TriggerGrid.Rows)
                {
                    ...
                    if (template.Triggers.IndexOf(template.EmailTemplateId, eventId,1) > -1)
                    {
    ...
    protected void SaveButton_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in TriggerGrid.Rows)
        {
            ...
            {
                int eventId = AlwaysConvert.ToInt(TriggerGrid.DataKeys[row.RowIndex].Value);
                template.Triggers.Add(new EmailTemplateTrigger(0, eventId,1));
            }
        }
        template.Save();
