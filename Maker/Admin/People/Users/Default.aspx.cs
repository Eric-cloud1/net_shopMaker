...
public partial class Admin_People_Users_Default : MakerShop.Web.UI.MakerShopAdminPage
{
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        GroupCollection storeGroups;
        int userId = Token.Instance.UserId;
       
        if (Token.Instance.User.IsAdmin)
            storeGroups = GroupDataSource.LoadForStore("Name");
        else
            storeGroups = GroupDataSource.LoadForUserRoles(userId);
        ...
        if (!Token.Instance.User.IsSystemAdmin)
        {
            ...
            foreach (MakerShop.Users.Group group in storeGroups)
            {
    ...
    private void LoadLastSearch()
    {
        ...
        if (criteria != null)
        {
            ...
            if (Token.Instance.User.IsGatewayAdmin)
                criteria.IsGateway = true;


            criteria.UserId = Token.Instance.UserId;
    ...
    private UserSearchCriteria GetSearchCriteria()
    {
        ...
        if (Token.Instance.User.IsGatewayAdmin) // GatewayAdmin
           criteria.IsGateway = true;
        ...
        int i;
        foreach (ListItem li in SearchGroup.Items)
        {
            if (int.TryParse(li.Value, out i))
                criteria.GroupIds.Add(i);
        }
    ...
    protected void SaveButton_Click(object sender, EventArgs e)
    {
        ...
        if (Page.IsValid)
        {
            ...
            if (ValidatePassword())
            {
                ...
                foreach (UserSetting s in Token.Instance.User.Settings)
                {
                    if (s.FieldName.ToLower().Contains("gateway"))
                    {
                        UserSetting userSetting = new UserSetting();
                        userSetting.FieldName = s.FieldName;
                        userSetting.FieldValue = s.FieldValue;
