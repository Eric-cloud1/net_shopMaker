...
public partial class Admin_Orders_Create_CreateOrder2 : MakerShop.Web.UI.MakerShopAdminPage
{
    private int _UserId;
    private User _User;
    ...
    protected BasketItem CreateBasketItem()
    {
        ...
        BasketItem basketItem = BasketItemDataSource.CreateForProduct(productId, tempQuantity, optionList, AlwaysConvert.ToList(",", _SelectedKitProducts), _UserId, PaymentTypes.Initial);
        if (basketItem != null)
        {
            //BASKET ID
        ...
        if (e.CommandName.Equals("Add"))
        {
            ...
            if (product != null)
            {
                ...
                else
                {
                    ...
                    BasketItem basketItem = BasketItemDataSource.CreateForProduct(productId, 1, String.Empty, String.Empty, _UserId,PaymentTypes.Initial);
                    if (basketItem != null)
                    {
                        _Basket.Items.Add(basketItem);
                        SaveBasket();
    ...
    protected void ShowProductForm(Product product)
    {
        ...
        _SelectedOptions = ProductHelper.BuildProductOptions(product, phOptions);
        //BUILD PRODUCT CHOICES
        ProductHelper.BuildProductChoices(product, phOptions);
        //BUILD KIT OPTIONS
        _SelectedKitProducts = ProductHelper.BuildKitOptions(product, phOptions);
        //SET PRICE
    ...
    protected void AddProductPrice_PreRender(object sender, EventArgs e)
    {
        ...
        if (product != null)
        {
            ...
            ProductCalculator pcalc = ProductCalculator.LoadForProduct(productId, 1, optionList, AlwaysConvert.ToList(",", _SelectedKitProducts), _UserId, PaymentTypes.Initial);
